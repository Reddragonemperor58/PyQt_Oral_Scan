...
class DentalArchGridVisualizerQt:
    ...
    def setup_scene(self):
        ...
        # Store the target 2D orientation parameters
        self.locked_camera_position = np.array(cam.GetPosition())
        self.locked_camera_focal_point = np.array(cam.GetFocalPoint())
        self.locked_camera_view_up = np.array(cam.GetViewUp())
        self.locked_camera_parallel_scale = cam.GetParallelScale()

        # Freezing focal point can still help reduce unwanted shifts during pan/zoom
    ...
    def _fit_camera_to_grid(self): 
        ...
        cam.SetViewUp(0,1,0)      # Ensure Y is up for a standard 2D view of XY plane
    ...
    def animate(self, timestamp_to_render): # Takes timestamp directly
        ...
        # current_timestamp_idx is mostly for the controller to iterate through self.processor.timestamps
        # This visualizer just renders the state for the given timestamp_to_render.
        # If you need current_timestamp_idx for some internal logic here, ensure it's set correctly.
        # For now, render_arch directly uses timestamp_to_render.
        self.render_arch(timestamp_to_render) # Updates actors on self.renderer

        # --- Enforce 2D Camera Lock for this view ---
        # This check is done after rendering the current frame's data.
        # If the user tried to rotate/tilt during the previous frame's display, this will snap it back.
        if self.parent_plotter and hasattr(self, 'locked_camera_position') and \
           hasattr(self, 'locked_camera_focal_point') and \
           hasattr(self, 'locked_camera_view_up') and \
           hasattr(self, 'locked_camera_parallel_scale'): # Ensure all locked params exist

            self.parent_plotter.at(self.renderer_index) # Activate this renderer's camera context
            cam = self.parent_plotter.camera
            
            current_focal_point_xy = np.array(cam.GetFocalPoint())[:2]
            locked_focal_point_xy = self.locked_camera_focal_point[:2]

            current_view_up = np.array(cam.GetViewUp())
            # Check if camera has significantly deviated from the locked 2D orientation
            # We primarily care if the view up vector has changed, or if the focal point's XY has (pan)
            # or if parallel scale has (zoom) - but zoom is allowed.
            # The position check relative to focal point also indicates rotation/tilt.
            
            # Calculate current direction vector from position to focal point
            current_pos = np.array(cam.GetPosition())
            current_fp = np.array(cam.GetFocalPoint())
            current_direction = current_fp - current_pos
            current_direction_xy_normalized = current_direction[:2] / (np.linalg.norm(current_direction[:2]) + 1e-6)

            locked_pos = self.locked_camera_position
            locked_fp = self.locked_camera_focal_point
            locked_direction = locked_fp - locked_pos
            locked_direction_xy_normalized = locked_direction[:2] / (np.linalg.norm(locked_direction[:2]) + 1e-6)

            # Check if view up changed or if the XY viewing direction changed significantly
            view_up_changed = not np.allclose(current_view_up, self.locked_camera_view_up, atol=1e-2)
            direction_changed = not np.allclose(current_direction_xy_normalized, locked_direction_xy_normalized, atol=1e-2)
            
            # Also check if Z component of focal point or position changed (indicates tilt)
            # For parallel projection, Z of focal point is less critical than Z of position for "looking straight"
            z_pos_changed = abs(current_pos[2] - self.locked_camera_position[2]) > 0.1 # Allow for some zoom-related Z shift

            if view_up_changed or direction_changed or z_pos_changed:
                logging.debug(f"Grid R{self.renderer_index}: Camera deviated, resetting to locked 2D view. "
                              f"VU_ch: {view_up_changed}, Dir_ch: {direction_changed}, ZPos_ch: {z_pos_changed}")
                cam.SetPosition(self.locked_camera_position)
                cam.SetFocalPoint(self.locked_camera_focal_point) # Reset full focal point
                cam.SetViewUp(self.locked_camera_view_up)
                cam.SetParallelScale(self.locked_camera_parallel_scale) # Restore zoom level too
                self.renderer.ResetCameraClippingRange()
                # No need to call self.renderer.ResetCamera() again if _fit_camera_to_grid did it well
                # and we are just restoring parameters.
                # The main window's render call will refresh the view.
        # --- End Enforce ---
